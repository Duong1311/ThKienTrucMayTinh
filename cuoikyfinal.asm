...
menu:	li   $v0, 51			# Lua chon Get hoac Set hoac thoat chuong trinh
	la   $a0, Message		
	syscall
	add  $a1, $0, $a0
	la   $a0,  WordPtr2
...
#  Khoang cua phan tu = (hang - 1)*Socot + cot-1
#------------------------------------------
SetArray:  
	jal  EnterRow
	nop
...
GetArray: 
	...
	lw   $t3, 0($a0)  		# Lay dia chi phan tu dau tien cua mang
	addi $t0, $t0, -1		# Hang - 1
	mul  $t0, $t0, $s2		# (Hang-1)*SoCot
	addi $t1, $t1, -1		# Cot-1
	add  $t0, $t0, $t1 		# (Hang-1)*SoCot + cot-1
